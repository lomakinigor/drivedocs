...
### T-103 — WaybillPreviewSheet (planned)
**Type:** impl | **Status:** planned | **Owner:** AI
Bottom sheet с предпросмотром данных путевого листа: заголовок с orgName/vehicle/driver, список строк-маршрутов, итого, блок warnings, кнопка "Скачать PDF" (disabled если !isExportReady). Открывается с TripsPage. Не реализуется в текущем mini-cycle.
**Files/Areas:** `src/features/trips/WaybillPreviewSheet.tsx` (planned)
**Links:** F-018, US-018, US-019, T-102, D-007, D-010

### T-104 — Client-side PDF generator (planned)
**Type:** impl | **Status:** planned | **Owner:** AI
Генерация PDF путевого листа на клиенте из `MonthlyWaybillData`. Кандидаты: `@react-pdf/renderer` или `jspdf`. Выбор библиотеки — отдельный decision (D-011 reserved). Не реализуется до утверждения preview sheet и дизайна шаблона.
**Files/Areas:** `src/features/trips/generateWaybillPdf.ts` (planned)
**Links:** F-018, US-018, T-103, D-010, D-011
